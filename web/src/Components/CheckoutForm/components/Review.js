...
function Review() {
    ...
    return (
        <>
            <Typography variant="h6" gutterBottom >Buyurtma narxi</Typography>
            <List disablePadding>
                {shopCartProducts.map(product => (
                   <ListItem className={classes.listItem} key={product.title}>
                        <ListItemText primary={product.title} secondary={`Qiymati: ${product.quantity}`} />
                        <Typography variant="body2">{product.price}</Typography>
                   </ListItem>
                ))}
                <ListItem className={classes.listItem}>
                        <ListItemText primary="Umumiy narx" />
